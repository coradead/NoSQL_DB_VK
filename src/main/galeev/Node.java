...
public abstract class Node {
    ...
    public abstract CompletableFuture<Iterator<Entry<String, Entry<Timestamp, byte[]>>>> get(String from, String to);
    ...
    public abstract CompletableFuture<Boolean> put(String key, Entry<Timestamp, byte[]> entry);
    ...
    public static class LocalNode extends Node {
        ...
        @Override
        public CompletableFuture<Iterator<Entry<String, Entry<Timestamp, byte[]>>>> get(String from, @Nullable String to) {
            Iterator<Entry<String, Entry<Timestamp, byte[]>>> iter = localGet(from, to);
            return CompletableFuture.completedFuture(iter);
        }

        protected Iterator<Entry<String, Entry<Timestamp, byte[]>>> localGet(String from, @Nullable String to) {
            return dao.get(from, to);
        }

        @Override
        public CompletableFuture<Entry<Timestamp, byte[]>> get(@Nullable String key) {
            Entry<Timestamp, byte[]> entry = localGet(key);
            return CompletableFuture.completedFuture(entry);
        }

        protected Entry<Timestamp, byte[]> localGet(@Nullable String key) {
        ...
        @Override
        public CompletableFuture<Boolean> put(String key, Entry<Timestamp, byte[]> entry) {
            localPut(key, entry);
            return CompletableFuture.completedFuture(true);
        }

        protected void localPut(String key, Entry<Timestamp, byte[]> entry) {
            dao.upsert(key, entry);
        }

        @Override
        public CompletableFuture<Boolean> delete(String key, Timestamp timestamp) {
            localDelete(key, timestamp);
            return CompletableFuture.completedFuture(true);
        }

        protected void localDelete(String key, Timestamp timestamp) {
        ...
        public CompletableFuture<Boolean> put(String key, Entry<Timestamp, byte[]> entry) {
            ...
            return httpClient.sendAsync(
                    requestBuilderForKey(nodeAddress, key).PUT(
                            HttpRequest.BodyPublishers.ofByteArray(entryToByteArray(entry.key(), entry.value()))
                    ).build(),
                    HttpResponse.BodyHandlers.ofByteArray()
            ).thenApply((response) -> {
