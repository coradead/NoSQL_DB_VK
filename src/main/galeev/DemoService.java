...
import static galeev.SkipOldExecutorFactory.shutdownAndAwaitTermination;
...
public class DemoService implements Service {
        ...
        for (Node node : routerNode) {
            handler.action(node, header.getKey()).thenAcceptAsync((optional) -> {
                        ...
                        if (barrier.isNeedToSendResponseToClient()) {
                            handlerResponse(session, handler, barrier);
                        }
                    },
                    proxyExecutor);
        }
    ...
    @SuppressWarnings("FutureReturnValueIgnored")
    private void handlerResponse(HttpSession session, Handler<?> handler, AckBarrier barrier) {
        try {
    ...
    public void localHandleGet(Request request, HttpSession session) throws IOException {
        ...
        Entry<Timestamp, byte[]> entry = localNode.localGet(key);
        if (entry.key() == null) {
